...
public class DatabaseManager {
    ...
    public boolean updateRequest(com.maintenance.models.MaintenanceRequest request) {
        String sql = "UPDATE maintenance_requests SET description = ?, category = ?, priority = ?, " +
                ...
                "completion_date = ?, staff_update_notes = ?, resolution_notes = ? WHERE request_id = ?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            pstmt.setTimestamp(5, request.getLastUpdated() != null
                    ? Timestamp.valueOf(request.getLastUpdated())
                    : null);
